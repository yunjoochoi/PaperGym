...
def sample_paper_grid(rows: list[dict], budget: int,
    ...
    Small pool (<= budget): keep everything. Large pool: take roughly
    budget/n_cells per cell, then top up to budget from leftovers
    ordered by citation rank.
    """
    if len(rows) <= budget:
        return list(rows)
...
# and publicationVenue.name strings. S2 venue strings are inconsistent
# (sometimes abbrev, sometimes full name, sometimes workshop suffix),
# so we keep both forms and match permissively.
