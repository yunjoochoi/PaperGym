...
Drives an tool-calling loop until the model stops
calling tools (natural end) or the step budget is exhausted. Tool
dispatch is provided by the caller as a dispatch(name, args) callable
returning an observation string, so this module is shared by the
Accumulator and any future agent.
"""
...
def run_tool_loop(
    *, llm: LLMClient, messages: list[dict], tools: list[dict],
    ...
    """Drive the native tool-calling loop until the agent stops calling
    tools (natural end) or max_steps is hit.

    dispatch(name, args) -> str routes each tool call to the caller's
    tool surface (e.g., PaperEnv.step for the Accumulator).
