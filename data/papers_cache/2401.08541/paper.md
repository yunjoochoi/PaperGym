...
Alaaeldin El-Nouby Alexander Toshev Michal Klein Vaishaal Shankar Shuangfei Zhai Joshua M Susskind
...
Prediction loss Our training objective naturally gives rise to certain variants of losses, each corresponding to a choice of the distribution P ( x k | x &lt;k ) . By default, we adopt a normalized pixel-level regression loss similar to He et al. [41]. This loss corresponds to setting P ( x k | x &lt;k ) as Gaussian distributions with a constant variance. Namely, given ˆ x k ( θ ) as the prediction of the k th patch from a network parameterized with θ , and x k as its corresponding ground-truth value, our objective is to minimize the sum ℓ 2 squared distance between the prediction and the ground-truth:
...
|----------|-----------|---------------|----------|---------|------------|--------------|
| AIM-0.6B | 0.6B      |          1536 |       24 | 1 e - 3 | 0.5T       |         4096 |
| AIM-1B   | 1.2B      |          2048 |       24 | 1 e - 3 | 1.2T       |         4096 |
| AIM-3B   | 2.7B      |          3072 |       24 | 1 e - 3 | 1.2T       |         4096 |
| AIM-7B   | 6.5B      |          4096 |       32 | 1 e - 3 | 1.2T       |         4096 |
...
Specifically, given a set of patch features P = { p i | 1 ≤ i ≤ K } , we compute a global descriptor ˆ p through multihead attention pooling over the patch features as:

<!-- formula-not-decoded -->

where for each attention head h = { 1 , ..., H } , W k h , W v h ∈ R d h × d correspond to the key and value weight matrices, respectively; q h is a learnable query vector. And we obtain the pooled feature as ˆ p = [ p 1 , ..., p H ] , ˆ p ∈ R d , which serves as the input to the linear classifier. By default, we set the number of heads H = d d h , which makes the total number of learnable parameters 2 d 2 + d , a negligible cost compared to the main model size. Including this attention pooling makes the entire operation not strictly linear, and, therefore we refer to it as 'Attentive Probe'. Nevertheless, the advantages of linear probing, e.g., low additional parameter count and a reduced risk of overfitting, are preserved with this probe.
...
|                    | (a) Targets .             | (a) Targets .             | (a) Targets .             | (a) Targets .             | (b) Autoregression Pattern (causal) . | (b) Autoregression Pattern (causal) . | (b) Autoregression Pattern (causal) . | (b) Autoregression Pattern (causal) . | (b) Autoregression Pattern (causal) . | (c) Crop Scale .   | (c) Crop Scale .   | (c) Crop Scale .   | (c) Crop Scale .   | (c) Crop Scale .   |
| pre-training attn. | causal                    | causal                    |                           |                           |                                       |                                       |                                       |                                       |                                       |                    |                    |                    |                    |                    |
...
|                    | (d) Attention Structure . | (d) Attention Structure . | (d) Attention Structure . | (d) Attention Structure . | (d) Attention Structure .             | (e) Head Design .                     | (e) Head Design .                     | (e) Head Design .                     | (e) Head Design .                     | (f) Architecture . | (f) Architecture . | (f) Architecture . | (f) Architecture . | (f) Architecture . |
...
| width         | 512           | 1024          |   2048 | depth           | 6               | 8               | 12              |
|---------------|---------------|---------------|--------|-----------------|-----------------|-----------------|-----------------|
| linear        | 69.4          | 69.6          |   70.0 | linear          | 65.3            | 68.1            | 70.0            |
| attentive     | 77.7          | 78.1          |   78.2 | attentive       | 76.2            | 77.1            | 78.2            |
| (a) MLP width | (a) MLP width | (a) MLP width |        | (b) MLP depth . | (b) MLP depth . | (b) MLP depth . | (b) MLP depth . |
...
| DINOv2 [58]  | ViT-g/14 516 | LVD     |    86.4 |      84.5 |      99.6 |       95.2 |      96.3 |  86.3 |   96.4 |   95.6 |       68.2 |         96.5 |   90.7 |     8.0 |      98.6 |   66.7 |          58.8 |  81.9 |
| BEiT [5]     | ViT-L/14     | IN-21k  |    62.2 |      44.4 |      94.4 |       78.7 |      79.0 |  64.0 |   80.9 |   69.5 |       52.0 |         92.8 |   88.2 |     4.2 |      97.5 |   47.7 |          25.9 |  65.4 |
| MAE [41, 70] | ViT-H/14     | IN-1k   |    80.9 |      64.6 |      97.1 |       85.8 |      90.2 |  78.1 |   95.0 |   93.7 |       58.1 |         94.2 |   89.8 |     5.4 |      98.1 |   56.9 |          42.2 |  75.3 |
|              | ViT-2B/14    | IG-3B   |    82.2 |      70.8 |      97.5 |       87.3 |      93.4 |  81.2 |   95.1 |   94.9 |       57.8 |         94.4 |   90.3 |     7.3 |      98.2 |   60.1 |          50.2 |  77.4 |
| AIM-0.6B     | ViT-H/14     | DFN-2B+ |    78.5 |      64.0 |      97.2 |       86.8 |      90.1 |  80.1 |   93.0 |   93.0 |       57.9 |         94.3 |   90.0 |     7.8 |      98.4 |   58.3 |          45.2 |  75.6 |
| AIM-1B       | ViT-1B/14    | DFN-2B+ |    80.6 |      67.2 |      98.2 |       88.3 |      91.6 |  81.8 |   93.4 |   93.9 |       58.6 |         94.5 |   90.0 |     9.0 |      98.6 |   59.8 |          47.5 |  76.9 |
| AIM-3B       | ViT-3B/14    | DFN-2B+ |    82.2 |      69.7 |      98.4 |       89.9 |      92.7 |  81.9 |   94.1 |   93.8 |       58.8 |         94.3 |   90.4 |     9.7 |      98.5 |   60.9 |          48.9 |  77.6 |
| AIM-7B       | ViT-7B/14    | DFN-2B+ |    82.4 |      70.9 |      98.6 |       90.0 |      93.1 |  82.3 |   93.8 |   92.1 |       59.5 |         93.6 |   90.7 |    10.1 |      98.6 |   61.7 |          49.6 |  77.8 |
...
16. national Conference on Computer Vision , pages 2959-2968, 2019. 2
...
- [45] Gao Huang, Yu Sun, Zhuang Liu, Daniel Sedra, and Kilian Q Weinberger. Deep networks with stochastic depth. In ECCV , 2016. 2, 4
- [46] iNaturalist 2018 competition dataset. iNaturalist 2018 competition dataset. https://github.com/visipedia/inat\_ comp/tree/master/2018 , 2018. 13
...
- [55] Mehdi Noroozi and Paolo Favaro. Unsupervised learning of visual representations by solving jigsaw puzzles. In ECCV , 2016. 2
...
|------------------|-----------|--------|-----------|
| Imagenet-1k [25] | 1,281,167 | 50,000 |      1000 |
| iNAT-18 [46]     |   437,513 | 24,426 |      8142 |
| CIFAR-10 [48]    |    50,000 | 10,000 |        10 |
| CIFAR-100 [48]   |    50,000 | 10,000 |       100 |
| Food101 [11]     |    75,750 | 25,250 |       101 |
| DTD [22]         |     3,760 |  1,880 |        47 |
| Pets [60]        |     3,680 |  3,669 |        37 |
| Cars [47]        |     8,144 |  8,041 |       196 |
| iWildCam [8]     |   129,809 |  14961 |       182 |
| Camelyon17 [4]   |   302,436 |  34904 |         2 |
| PCAM [80]        |   262,144 |  32768 |         2 |
| RxRx1 [71]       |    40,612 |   9854 |      1139 |
| EuroSAT [42]     |    16,200 |   5400 |        10 |
| fMoW [21]        |    76,863 |  19915 |        62 |
| Infograph [63]   |    36,023 | 15,582 |       345 |
...
In Tables 7 and 6, we discussed the gain in downstream performance that can be achieved by probing shallower layers in the model rather than the last. We study this in more detail in Figure 10. We find that for all AIM variants, we extract the highest quality features, with respect to the downstream transfer, from layers roughly at two-thirds of the way into the model depth. However, it is important to note that the performance of deeper layers does not experience a steep decline and continues to exhibit strong performance.

Figure 8. Autoregression patterns. Weillustrate the different autoregression patterns studied in this work including raster, spiral, checkerboard, and fixed random.
...
| config                 | value                     | config                          | IN-1k Others                        |
|------------------------|---------------------------|---------------------------------|-------------------------------------|
| Optimizer              | AdamW [52]                | Optimizer                       | AdamW [52]                          |
| Optimizer Momentum     | β 1 = 0 . 9 ,β 2 = 0 . 95 | Optimizer Momentum              | β 1 = 0 . 9 ,β 2 = 0 . 999          |
| Peak learning rate     | 1 e - 3                   | Peak learning rate grid         | [1, 3, 5, 10, 15, 20, 40] × 1 e - 4 |
| Minimum Learning rate  | 0.0                       | Minimum Learning rate           | 1 e - 5 0.1                         |
| Weight decay           | 0.05                      | Weight decay                    |                                     |
| Batch size             | 4096                      | Batch size                      | 1024 512                            |
| Patch size             | (14, 14)                  | Gradient clipping Warmup epochs | 3.0 5                               |
| Gradient clipping      | 1.0                       | Epochs                          | 0 50 100                            |
| Warmup iterations      | 31,250                    | Learning rate schedule          | cosine decay [51]                   |
| Total iterations       | 1,250,000                 | Augmentations:                  |                                     |
| Learning rate schedule | cosine decay [51]         | RandomResizedCrop               |                                     |
| Augmentations:         |                           | size                            | 224px                               |
| RandomResizedCrop      |                           | scale                           | [0.08, 1.0]                         |
| size                   | 224px                     | ratio                           | [0.75, 1.33]                        |
| scale                  | [0.4, 1.0]                | interpolation                   | Bicubic                             |
| ratio                  | [0.75, 1.33]              | RandomHorizontalFlip            | p = 0 . 5                           |
| interpolation          | Bicubic                   | Color Jitter                    | 0.3                                 |
| RandomHorizontalFlip   | p = 0 . 5                 | AutoAugment [23]                | rand-m9-mstd0.5-inc1                |

Attentive Probing. Downstream evaluation for AIM and the baselines has been primarily conducted via attentive probing as described in § 4. We report the hyperparameters used to probe all methods in Table 11. For a fair comparison with other baselines, we search over different values for the learning rate and report the best performance of each method similar to [58]. For AIM and other generative baselines, we average the features for the last 6 layers of the model before feeding them to the attention-probing head which leads to a modest gain in performance. Note that the descriptor dimensionality remains the same which is different from the practice of concatenating features similar to iGPT[18] which indirectly inflates the capacity of the evaluation head.
